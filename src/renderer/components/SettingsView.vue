          ...
          <section v-show="activeTab === 'about'" class="settings-section">
            ...
            <div class="about-info">
              <div class="about-logo">
                ...
                />
                <div v-if="logoLoadFailed" class="about-logo-placeholder">M</div>
              </div>
              <p class="app-name">Message</p>
              <p class="version">版本 {{ appStore.version || '—' }}</p>
              <p class="description">局域网点对点即时通讯工具</p>
              <div class="about-divider"></div>
              <p class="copyright">Copyright © 2026 Message</p>
            </div>
          </section>
        </div>
      </div>
...
onMounted(async () => {
  ...
  if (config) {
    ...
    // 如果没有设置 userDataDir，显示当前实际使用的目录（受 MESSAGE_DATA_DIR 环境变量影响）
