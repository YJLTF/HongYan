...
- **好友自动发现** - 通过 UDP 广播自动发现同一局域网内的其他鸿雁用户，支持手动扫描指定网段
...
- **离线检测** - 15 秒无心跳自动判定好友离线
- **现代化UI** - 类似微信的专业通讯工具界面，支持头像、表情选择器、时间分组等
- **好友备注** - 支持为联系人设置自定义备注名
...
| `BROADCAST_INTERVAL_MS` | 5000 | 广播间隔（5 秒） |
| `ONLINE_TIMEOUT_MS` | 15000 | 在线超时（15 秒） |
...
| 文件 | 说明 |
|------|------|
| `hongyan.db` | SQLite 数据库（聊天记录、好友、文件传输记录） |
| `master.key` | 主密钥文件 |
