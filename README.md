...
- **好友自动发现** — 事件驱动 UDP 广播，同一局域网内自动感知其他阿卜兹的用户
...
9. 关闭主窗口默认**最小化到系统托盘**；通过托盘菜单的「显示主窗口 / 退出阿卜兹的」唤起或彻底退出
...
- 内网部署建议配置防火墙允许上述端口通信
- 如需修改默认端口，可通过环境变量 `ABCD_UDP_PORT` 和 `ABCD_TCP_PORT` 指定
- 如需修改数据目录，可通过设置页「自定义数据目录」或环境变量 `ABCD_DATA_DIR` 指定
- `ABCD_DATA_DIR` 的优先级高于 `identity.json` 里的 `userDataDir` 设置（用于多实例测试隔离）
...
| 常量 / Getter | 默认值 | 说明 |
|------|----|------|
| `getUdpPort()` | 19876 | UDP 广播端口（可被 `ABCD_UDP_PORT` 覆盖） |
| `getTcpPortDefault()` | 19877 | TCP 起始端口（可被 `ABCD_TCP_PORT` 覆盖） |
| `getTcpPortMax()` | `getTcpPortDefault() + 9` | TCP 端口范围上限 |
| `getAppDataDir()` | `Abcd` | 数据目录名（可被 `ABCD_DATA_DIR` 覆盖） |
...
| `ABCD_UDP_PORT` | 自定义 UDP 端口 | 19876 |
| `ABCD_TCP_PORT` | 自定义 TCP 起始端口 | 19877 |
| `ABCD_DATA_DIR` | 自定义数据目录名（追加在 `%APPDATA%/` 下）；设置后自动进入多实例模式 | `Abcd` |

---

## 数据存储

应用数据目录：`%APPDATA%/<ABCD_DATA_DIR>/`（默认 `Abcd`）

> 优先级：`ABCD_DATA_DIR` 环境变量 > `identity.json` 中的 `userDataDir` > 默认 `Abcd`

| 文件 | 说明 |
|------|------|
| `abcd.json` | lowdb 数据库（聊天记录、好友、文件传输记录、群组、发布/可用更新） |
...
|------|----------|----------|----------|
| Instance 1 | `%APPDATA%/Abcd-Test1/` | 19876 | 19877 |
| Instance 2 | `%APPDATA%/Abcd-Test2/` | 19878 | 19879 |
| Instance 3 | `%APPDATA%/Abcd-Test3/` | 19880 | 19881 |

### 工作原理

- **数据隔离**：`ABCD_DATA_DIR` 优先级最高，覆盖 `identity.json` 里的 `userDataDir` 设置，让三个实例的数据库/主密钥/日志完全分开
- **端口隔离**：每个实例绑定独立的 UDP/TCP 端口，不会冲突
- **自动发现**：检测到 `ABCD_DATA_DIR` 被设置时，自动切换到多实例广播模式
...
```bash
rd /s /q "%APPDATA%\Abcd-Test1"
rd /s /q "%APPDATA%\Abcd-Test2"
rd /s /q "%APPDATA%\Abcd-Test3"
```
...
```json
{
  ...
  "data": {
    ...
    "httpPort": 19890,
    "nsis":     { "filename": "Abcd-1.5.0-x64.exe",     "size": 12345678, "sha256": "<hex>" },
    "portable": { "filename": "Abcd-1.5.0-portable.exe", "size": 12345678, "sha256": "<hex>" },
    "note": "修复群文件下载问题",
    "timestamp": 1700000000000,
    "stopped": false,
    "publicKey": "<base64 Ed25519 SPKI>",
...
3. 主进程 `update-downloader.ts` 通过 `http.get` 流式下载到 `%TEMP%/AbcdUpdate/<jobId>/`
...
| 修复 | 说明 |
|---|---|
| **群密钥磁盘持久化** | 原实现群密钥仅存内存，应用重启后丢失，发文件报 `No group key available`。改为 AES-256-GCM 加密存到 `${dataDir}/group-keys.json`（用 master key + aad `abcd-group-keys-v1`），启动时 `loadAllGroupKeys()` 恢复 |
...
| AppUserModelID | 未设置（Windows Toast 归到 electron.exe）| `com.abcd.messenger`，toast 归到 Abcd 名下 |
